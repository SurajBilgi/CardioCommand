...
def build_system_prompt(patient: dict, script: dict, outreach_script: str = None) -> str:
    """
    ...
    If outreach_script is provided (AI-generated), it is used instead of generic questions.
    """
    if outreach_script:
        questions_block = (
            "PERSONALIZED CALL PLAYBOOK (generated by clinical AI based on this patient's current status "
            "and doctor guidance — follow this closely and present any missions naturally):\n"
            f"{outreach_script}"
        )
    else:
        questions_block = "QUESTIONS TO COVER (ask naturally, one at a time — do NOT rush):\n" + "\n".join(
...
def build_assistant_config(patient: dict, outreach_script: str = None) -> dict:
    """
    ...
    return {
        ...
        "model": {
            ...
            "maxTokens": 180,
            "messages": [
                {"role": "system", "content": build_system_prompt(patient, script, outreach_script)}
            ],
        },
        "voice": {
            "provider": "11labs",
...
async def create_assistant(patient: dict, outreach_script: str = None) -> dict:
    ...
    config = build_assistant_config(patient, outreach_script)
